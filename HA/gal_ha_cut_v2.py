...
def ha_xreg_match(primary_dir, file_cut_NB, file_cut_BB, file_UV, i):
    ...
    fNB_BB_file = file_cut_NB.replace("scale_04_cut_drc", "NB_BB_align")
    fNB_align_file = file_cut_NB.replace("scale_04_cut_drc", "NB_UV_align")
    fBB_align_file = file_cut_BB.replace("scale_04_cut_drc", "BB_UV_align")
    #f1 = f782_775_file + "[0]"
    ...
    iraf.xregister(file_cut_NB + "[1]", file_UV + "[1]", '[%s:%s, %s:%s]' % (win_x_lw, win_x_hi, win_y_lw, win_y_hi), shift_NB,
                   out=fNB_align_file, function='sawtooth', xwindow=100, ywindow=100, interp_type='nearest')

    # align BB to UV reference

    iraf.xregister(file_cut_BB + "[1]", file_UV + "[1]", '[%s:%s, %s:%s]' % (win_x_lw, win_x_hi, win_y_lw, win_y_hi), shift_BB,
                   out=fBB_align_file, function='sawtooth', xwindow=100, ywindow=100, interp_type='nearest')
...
def ha_cont_sub(primary_dir, fNB_align_file, fBB_align_file):
    ...
    hdulist_BB[1].data = (hdulist_NB[0].data - hdulist_BB[0].data)
    hdulist_BB[1].header = head_Ha
