...
def sky_dark_sub(gal_num, configfile, section, section_gal, show_output, dark_RAW, dark_FLT, temp , dark_perform):
    ...
    for i in range(len(tab)):
        ...
        and tab["channel"][i] =="SBC":
            ...
            #print (gal_key, sky_value[i])

            #sky_value[i] = np.mean (aper_annulus[350:400]) ##380:450
            #print (gal_key, sky_value[i])

            #sky_value[i] = np.mean (aper_annulus[450:500]) ##380:450
            #print (gal_key, sky_value[i])


            #sky_value[i] = np.mean (aper_annulus[400:500]) ##380:450
...
def function_dark_sub(a,k, aper_diff_gal, aper_diff_dark, dark_radii, rad1, galaxy, dark, masks_annulus):
    ...
    #s2 = np.mean(aper_diff_ann[dark_radii:len(rad1)-1])
    #s3 = np.mean(aper_diff_ann[0:len(rad1)-1])
    #s4 = np.mean(aper_diff_ann[0:len(rad1)-1])
