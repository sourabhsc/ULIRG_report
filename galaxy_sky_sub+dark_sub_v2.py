...
def sky_dark_sub(gal_num, configfile, section, section_gal, show_output, dark_RAW, dark_FLT, temp):
    ...
    for i in range(len(tab)):
       ...
       'white', attrs=['bold'])
            ...
            dark_sub(file_name_flt, file_mask, dark_FLT, dark_RAW, temp, params, params_gal, gal_num )
            if show_output == "True":
                print ("dark subtraction done for ", file_name_flt)
...
def dark_sub(file_name_flt, file_mask, dark_FLT, dark_RAW, temp, params, params_gal, gal_num):
    ...
    dark_radii=float(params_gal["dark_radii"])
    ...
    for i in range(len(dark_RAW)):
        ...
        fits_dark_name = file_name_no_dir.replace("flt.fits", "dark_%s_v4.fits"%(i+1))
        ...
        fits_variance_name = file_name_no_dir.replace("flt.fits", "dark_%s_diff_v4.fits"%(i+1))
    ...
    minimizer_png = file_name_no_dir.replace("flt.fits", "minimizer_v4.png")
    ...
    dark_selected = primary_dir+ file_name_no_dir.replace("flt.fits", "dark_%s_v4.fits"%(ind+1)) 
    ...
    sub_name = file_name_flt.replace("flt.fits", "drk_flt_v4.fits" )
    ...
    ax2.legend()
    verification_png = file_name_no_dir.replace("flt.fits", "verification_v4.png")
